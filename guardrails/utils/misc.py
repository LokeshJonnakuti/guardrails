...
def generate_random_schemas(n: int, depth: int = 4, width: int = 10) -> List[str]:
    ...
    def random_scalar_datatype():
        selected_datatype = secrets.choice([
                dt.String,
                dt.Integer,
                dt.Float,
                dt.Boolean,
    ...
    def generate_schema(curr_depth):
        if curr_depth < depth:
            ...
            node_type = secrets.choice(["object", "list", "scalar"])

            if node_type == "object":
                # If "object", then generate random number of children
                num_children = secrets.SystemRandom().randint(1, width)
                children = []
                for _ in range(num_children):
                    children.append(generate_schema(curr_depth + 1))
                return E.object(
                    *children, name=f"random_object_{secrets.SystemRandom().randint(0, 1000)}"
                )
            elif node_type == "list":
                # If "list", then generate a single child
                return E.list(
                    generate_schema(curr_depth + 1),
                    name=f"random_list_{secrets.SystemRandom().randint(0, 1000)}",
                )
            else:
                # If "scalar", then return a random primitive type
                datatype = random_scalar_datatype()
                return E(datatype, name=f"random_{datatype}_{secrets.SystemRandom().randint(0, 1000)}")
